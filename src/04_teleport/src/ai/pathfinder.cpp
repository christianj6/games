...
PathNodePool::PathNodePool(unsigned _allocate, unsigned _typicalAdjacent)
    : firstBlock(0), blocks(0),
#if defined(MICROPATHER_STRESS)
      allocate(32),
#else
      allocate(_allocate),
#endif
      nAllocated(0), nAvailable(0) {
...
void MicroPather::DumpStats()
{
        ...
        for( PathNode* node = pathNodeMem; node; node = node[ALLOCATE-1].left )
                ++pathNodeBlocks;
        printf( "HashTableEntries=%d/%d PathNodeBlocks=%d [%dk] PathNodes=%d
...
#ifdef DEBUG_CACHING
      GLOUTPUT(
          ("Add: start=%x next=%x end=%x\n", item.start, item.next, item.end));
#endif
      break;
    } else if (mem[index].KeyEqual(item)) {
      MPASSERT((mem[index].next && item.next) ||
               (mem[index].next == 0 && item.next == 0));
      // do nothing; in cache
      break;
    }
    ++index;
  ...
  */
  ...
  while (!open.Empty()) {
    ...
    for (int i = 0; i < node->numAdjacent; ++i) {
      ...
      PathNode *child = nodeCostVec[i].node;
      MPASSERT(child->state !=
               newPathNode->state); // should never re-process the parent.
