 ...
 * between waves, NPCs periodically request assets which are used to attack enemies
 * player must explore the open world to find the assets, while avoiding enemies
 * assets can be used to create traps, or NPCs throw/shoot them
 * player can attack enemies by getting close but it is very risky
 * NPCs can heal the player between waves
 * if all NPCs die you lose
 * if make it through all the waves you win
 */
