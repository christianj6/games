...
// enum class means we are enforcing stricter typing on the enum members, as
// opposed to a c-style class
...
private:
  ...
  // size_t is unsigned int which is as large as any obj in memory; used
  // typically to represent container sizes
...
public:
  ...
  // syntax here means we are passing a vector of game object pointers by
  // reference, promising not to change them
  bool
  is_player_in_vision_cone(const Vector2 &player_pos,
  ...
  bool has_line_of_sight(const Vector2 &target_pos,
                         const std::vector<GameObject *> &obstacles) const;
};
